...
namespace SimSnakeAndLadder
{
    class Board
    {
        ...
        private int CreateRandomNumber(int start,int end)//creates a random number between start and end-1
        {
            Random rnd = new Random();
            int rand = rnd.Next(start, end);   // creates a number between start and end-1
            return rand;
        }
        ...
        private void CreateSnakesOrLadders(int amount,TypeCell type)//create tiles of snakes or ladders by type and amount.
        {
            ...
            for (int i = 0; i < amount; i++)
            {
                ...
                if (type==TypeCell.Ladder)
                {
                    while (cellsArray[cellFrom].Type.Key != TypeCell.Regular || cellFrom / 10 == (size ) / 10)
                    {
                        cellFrom = CreateRandomNumber(0, size);
                    }
